...
def MaskNet(categorical_feats: List[str], vocab_sizes: List[int], numerical_feats: List[str], n_outputs: int,
            is_serial: bool):
    if is_serial:
    ...
    embeddings = [layers.Embedding(vocab_sizes[feat_idx], embeddings_len)(emb_input) for feat_idx, emb_input in
                  enumerate(emb_inputs)]
    ...
    mask_net_model = Model(inputs=[*emb_inputs, *proj_inputs], outputs=outputs,
                           name='SerialMaskNet' if is_serial else 'ParallelMaskNet')
